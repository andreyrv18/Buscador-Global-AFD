document.addEventListener("DOMContentLoaded", () => {
  ...
  const controlesPaginacaoEventos = document.getElementById(
    "controlesPaginacaoEventos",
  );
  ...
  const selectTamanhoPaginaEventos = document.getElementById(
    "tamanhoPaginaEventos",
  );
  ...
  const resumoValoresMarcacoes = document.getElementById(
    "resumoValoresMarcacoes",
  );

  const controlesPaginacaoMarcacoes = document.getElementById(
    "controlesPaginacaoMarcacoes",
  );
  ...
  const selectTamanhoPaginaMarcacoes = document.getElementById(
    "tamanhoPaginaMarcacoes",
  );
  ...
  function notificarConclusao() {
    if (typeof chrome !== "undefined" && chrome.notifications) {
      chrome.notifications.create("", {
        ...
        title: "AFD Concluída!",
        message: "O processamento do arquivo foi finalizado com sucesso.",
      });
    }
  }
  ...
  function aplicarFiltrosEOrdenacao() {
    ...
    registrosFiltrados = todosRegistros.filter((reg) => {
      ...
      const bateOperacao =
        operacaoSelecionada === "Todas" || reg.operacao === operacaoSelecionada;
  ...
  function renderizarResumo() {
    ...
    const incFiltrado = registrosFiltrados.filter(
      (r) => r.operacao === "Inclusão",
    ).length;
    const altFiltrado = registrosFiltrados.filter(
      (r) => r.operacao === "Alteração",
    ).length;
    const excFiltrado = registrosFiltrados.filter(
      (r) => r.operacao === "Exclusão",
    ).length;
  ...
  function renderizarPagina() {
    ...
    if (registrosPagina.length === 0) {
      ...
      const datasUnicas = [
        ...new Set(registrosFiltrados.map((r) => r.dataHora)),
      ];
  ...
  btnProximoEventos.addEventListener("click", () => {
    const totalPaginas = Math.ceil(
      todosEventos.length / registrosPorPaginaEventos,
    );
  ...
  function renderizarPaginaEventos() {
    ...
    const datasUnicas = [
      ...new Set(todosEventos.map((e) => e.dataHora.split(" ")[0])),
    ];
    ...
    btnProximoEventos.disabled =
      paginaAtualEventos >= totalPaginas || totalPaginas === 0;
  }
  ...
  function renderizarPaginaMarcacoes() {
    ...
    const totalPaginas = Math.ceil(
      marcacoesFiltradas.length / registrosPorPaginaMarcacoes,
    );
    const inicio = (paginaAtualMarcacoes - 1) * registrosPorPaginaMarcacoes;
    const registrosPagina = marcacoesFiltradas.slice(
      inicio,
      inicio + registrosPorPaginaMarcacoes,
    );
    ...
    registrosPagina.forEach((reg) => {
      const tr = document.createElement("tr");
      if (datasUnicas.indexOf(reg.dataHora) % 2 !== 0)
        tr.classList.add("linha-zebra");
    ...
    btnProximoMarcacoes.disabled =
      paginaAtualMarcacoes >= totalPaginas || totalPaginas === 0;
  }

  filtroNomeMarcacoes.addEventListener(
    "input",
    aplicarFiltrosEOrdenacaoMarcacoes,
  );
  filtroCpfMarcacoes.addEventListener(
    "input",
    aplicarFiltrosEOrdenacaoMarcacoes,
  );
  filtroDataMarcacoes.addEventListener(
    "change",
    aplicarFiltrosEOrdenacaoMarcacoes,
  );
  ordemDataMarcacoes.addEventListener(
    "change",
    aplicarFiltrosEOrdenacaoMarcacoes,
  );
